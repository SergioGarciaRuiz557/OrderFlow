...
import com.orderflow.order.application.port.in.HandleInventoryReleasedUseCase;
...
@Service
public class HandleInventoryReleasedService implements HandleInventoryReleasedUseCase {
    private final OrderRepository repository;
    private final IntegrationMessagePublisher publisher;
    private final ClockProvider clock;

    public HandleInventoryReleasedService(OrderRepository repository, IntegrationMessagePublisher publisher, ClockProvider clock) {
        this.repository = repository; this.publisher = publisher; this.clock = clock;
    }

    @Override @Transactional public void handle(UUID orderId) {
        Order order = OrderApplicationSupport.load(repository, new OrderId(orderId));
        order.confirmCancellation("PAYMENT_REJECTED", clock.now());
